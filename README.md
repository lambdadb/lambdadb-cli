...
Input must be a UTF-8 regular file of at most 64 MiB. Each nonblank line must be
a JSON object. If supplied, `id` must be a nonempty string. Omitted IDs are
...
cost of buffering the input. Peak memory exceeds the raw file size. Batches run
